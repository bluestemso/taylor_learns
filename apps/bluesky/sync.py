...
def run_sync(*, limit: int = 100) -> dict[str, int]:
    ...
    lock_expires_at = started_at + timedelta(minutes=10)
    with transaction.atomic():
        lock_acquired = (
            BlueskySourceSettings.objects.filter(id=source_settings.id, is_active=True, is_enabled=True)
            .filter(Q(sync_lock_expires_at__isnull=True) | Q(sync_lock_expires_at__lt=started_at))
            .update(sync_lock_token=lock_token, sync_lock_expires_at=lock_expires_at)
        )
    ...
    try:
        ...
        BlueskySourceSettings.objects.filter(id=source_settings.id, sync_lock_token=lock_token).update(
            sync_lock_token=None,
            sync_lock_expires_at=None,
        )
