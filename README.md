...
The personal blog and portfolio of Taylor Schaack
...
Create a database named `taylor_learns`.

```
createdb taylor_learns
```
...
```bash
celery -A taylor_learns worker -l INFO --pool=solo
```

Or with celery beat (for scheduled tasks):

```bash
celery -A taylor_learns worker -l INFO -B --pool=solo
```
